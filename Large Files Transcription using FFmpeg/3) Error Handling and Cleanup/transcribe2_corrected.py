...
# correction: chunks = split_media(file_path, 1) # 1Mb chunks
...
def get_audio_duration(file_path):
    ...
    try:
        ...
        return float(output.decode().strip())       #subprocess.check_output returns bytes in Python, and float(b"12.34\n") will raise a TypeError. You should decode and strip first
    except:
        return None
...
def transcribe(file_path):
    ...
    chunks = []
    try:
        chunks = split_media(file_path, 1) # 1Mb chunks
        
        if not chunks:
            print("File small enough, transcribing without splitting...")
            return transcribe_small_media(file_path)            
